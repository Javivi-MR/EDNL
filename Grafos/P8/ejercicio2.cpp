...
matriz<double> tombuctu2(Grafo& G, vector<ciudad> Vciudad);
...
int main ()
{
    ...
    matriz<double> res = tombuctu2(G,Vciudad); 
...
matriz<double> tombuctu2(Grafo& G, vector<ciudad> Vciudad)
{
    ...
    for(vertice i = 0 ; i < n ; i++)
    {
        for(vertice j = 0 ; j < n ; j++)
        {
            if(G[i][j])
            {
                Gresultado[i][j] = sqrt(pow(Vciudad[i].x - Vciudad[j].x,2) + pow(Vciudad[i].y - Vciudad[j].y,2));
